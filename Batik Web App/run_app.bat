...
- **المعينات والزجزاج**: تُستخدم غالبًا في حدود السارونغ (كيبالا)، وتوفر إطارًا هيكليًا للتصميمات المركزية الأكثر سيولة
...
الأنماط الهندسية في باتيك ماليزيا هي أكثر من مجرد عناصر زخرفية—إنها رياضيات بصرية تربط مرتديها بالمبادئ الروحية والتراث الثقافي والجمال المنظم لكل من العالم الطبيعي والخليقة الإلهية."""
        ...
        with audio_col1:
            if st.button("▶️ Play Audio Story", use_container_width=True):
                with st.spinner("Generating audio..."):
                    time.sleep(1.5)
                st.success(f"Playing story in {language}...")
                st.balloons()
        with audio_col2:
            if st.button("💾 Save Audio", use_container_width=True):
                st.success("Audio saved to: output/audio_stories/")
        with audio_col3:
        ...
        with action_col1:
            if st.button("🔄 Analyze Another", use_container_width=True):
                for key in ['analyzing', 'use_sample', 'image_uploaded']:
        ...
        with action_col3:
            if st.button("📁 Save Report", use_container_width=True):
                st.success("Report saved to output/story_logs/")
