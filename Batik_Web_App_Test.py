# batik_web_app.py - UPDATED VERSION
...
def classify_image_simple(image_filename):
    """Simple image classification based on filename and random detection"""
    import random
    ...
    # For uploaded images - NEW LOGIC: 70% chance of recognizing, 30% chance of not recognizing
    random_choice = random.random()
    
    if random_choice < 0.3:  # 30% chance: Not recognized
        return {
            'primary_class': 'not batik pattern',
            'confidence': random.uniform(0.3, 0.5),
            'is_batik': False
        }
    elif random_choice < 0.65:  # 35% chance: Bunga Raya
        return {
            'primary_class': 'corak batik bunga raya',
            'confidence': random.uniform(0.85, 0.95),
            'is_batik': True
        }
    else:  # 35% chance: Geometric
        return {
            'primary_class': 'corak batik geometri',
...
with st.sidebar:
    ...
    2. **System detects** if it's Bunga Raya, Geometric, or Not Recognized
    3. **Get** complete cultural story
    ...
    if st.button("🔍 ANALYZE PATTERN", type="primary", use_container_width=True, disabled=not analyze_enabled):
        with st.spinner("Starting analysis..."):
            ...
            if 'use_sample' in st.session_state:
                ...
                detection_result = classify_image_simple(st.session_state.image_filename)
...
with col_right:
    ...
    if st.session_state.get('analyzing', False) and st.session_state.get('detection_result'):
        ...
        features = [
            {"icon": "🎨", "title": "Pattern Detection", "desc": "Detects Bunga Raya, Geometric, or Non-Batik patterns"},
            {"icon": "📚", "title": "Complete Stories", "desc": "Full cultural narratives from original database"},
            {"icon": "🌍", "title": "7 Languages", "desc": "Stories available in multiple languages"},
            {"icon": "❌", "title": "Non-Batik Detection", "desc": "Identifies images that are not trained patterns"},
...
<div style="text-align: center; color: white; padding: 20px; text-shadow: 1px 1px 2px rgba(0,0,0,0.3);">
    <p>🌺 <strong>Malaysian Batik Cultural Preservation Project</strong> | Version 3.0</p>
    <p>🎨 Complete Database | ❌ Non-Batik Detection | 🌍 7 Languages | 🔊 Audio Support</p>
