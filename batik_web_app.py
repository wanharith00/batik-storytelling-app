...
with st.sidebar:
    ...
    # === ADDED: Save Location Info in Sidebar ===
    st.divider()
...
히비스커스는 말레이시아の国花이며 바틱 디자인に 깊은文化적 의미를 지니고 있습니다. 이 꽃 패턴은 다음을 상징합니다:
...
几何图案象征无限和神性。重复性质反映冥想和精神沉思。东海岸蜡染中常见的这些图案展示了马来-伊斯兰艺术融合。"""
            ...
            with audio_col2:
                ...
                if st.button("📁 Save Report", use_container_width=True):
                    try:
                        ...
                        st.write("")
                        st.info("💡 Files are saved in your 'Batik Web App' folder on Desktop")
                        st.markdown('</div>', unsafe_allow_html=True)
                        # ========== END ADDED SECTION ==========
        ...
        with action_col1:
            if st.button("🔄 Analyze Another", use_container_width=True):
                # Clear session state
                for key in ['analyzing', 'use_sample', 'image_uploaded', 'audio_bytes', 'current_story', 'pattern_name', 'confidence', 'audio_filename']:
