...
# Custom CSS with CLEAR VISIBLE TEXT
st.markdown("""
<style>
    /* Main title */
    .main-title {
    ...
    /* Subheaders - DARK TEXT for visibility */
    .section-header {
    ...
    /* Language buttons - SIMPLE NO BOXES */
    .language-button {
        background-color: #f8f9fa;
        border: none;
        border-radius: 5px;
    ...
    /* Remove default Streamlit widget borders */
    .stButton > button {
...
with col_left:
    ...
    if uploaded_file is not None:
        # Display uploaded image - FIXED: removed use_column_width
...
with col_right:
    ...
    if st.session_state.get('analyzing', False):
        ...
        # PATTERN DETECTED SECTION - CLEAR DARK TEXT
        st.markdown('<div class="pattern-header">🎨 Pattern Detected</div>', unsafe_allow_html=True)
        st.markdown(f'<h2 style="color: #FF6B6B; margin-bottom: 10px;">{pattern_name}</h2>', unsafe_allow_html=True)
        
        # Confidence with DARK TEXT
        ...
        # SELECTED LANGUAGE - SIMPLE BUTTONS NO BOXES
        st.markdown('<div class="section-header">Selected Language</div>', unsafe_allow_html=True)
        
        # Language buttons - SIMPLE 3-LETTER CODES
        languages = ["English", "Malay", "Indonesian", "Arabic", "Japanese", "Korean", "Chinese"]
        lang_codes = ["Eng", "Mal", "Ind", "Ara", "Jap", "Kor", "Chi"]
        
        cols = st.columns(7)
        
        for idx, (lang, code) in enumerate(zip(languages, lang_codes)):
            with cols[idx]:
                # Create simple button without box
                if st.button(
                    f"{code}", 
                    key=f"lang_{lang}",
        ...
        # CULTURAL STORY SECTION - CLEAN TEXT NO MARKDOWN
...
<div style="text-align: center; color: #666; padding: 20px;">
    <p style="color: #333;">🌺 <strong>Malaysian Batik Cultural Preservation Project</strong> | Version 2.0</p>
    <p style="color: #555;">🧠 AI-Powered Pattern Recognition | 🎨 Cultural Storytelling | 🌍 Multilingual Support</p>
    <p style="color: #555;">📧 Contact: cultural.heritage@batik.edu.my | 📱 +60 12-345 6789</p>
</div>
""", unsafe_allow_html=True)