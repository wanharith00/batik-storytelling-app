...
with col_left:
    st.markdown('<h2 class="clear-text">Step 1: Upload Image</h2>', unsafe_allow_html=True)
...
with col_right:
    st.markdown('<h2 class="clear-text">Step 2: Story Results</h2>', unsafe_allow_html=True)
    ...
    if st.session_state.get('analyzing', False):
        ...
        # PATTERN DETECTED SECTION - CLEAR TEXT
        st.markdown('<div class="pattern-detected-title">Pattern Detected</div>', unsafe_allow_html=True)
        st.markdown(f'<h1 style="color: #FF6B6B;">{pattern_name}</h1>', unsafe_allow_html=True)
        ...
        # SELECTED LANGUAGE (Simple buttons without boxes)
        st.markdown("### Selected Language")
        
        # Language buttons - SIMPLE TEXT BUTTONS
        languages = ["English", "Malay", "Indonesian", "Arabic", "Japanese", "Korean", "Chinese"]
        language_codes = ["Eng", "Mal", "Ind", "Ara", "Jap", "Kor", "Chi"]
        
        cols = st.columns(7)
        
        for idx, (lang, code) in enumerate(zip(languages, language_codes)):
            with cols[idx % 7]:
                if st.button(f"{code}", 
                           key=f"lang_{lang}",
        ...
        # CULTURAL STORY SECTION - NO MARKDOWN FORMATTING
        st.markdown("### Cultural Story")
        
        # Define CLEAN stories (no ** or * formatting)
        if "Bunga" in pattern_name:
            stories = {
...
几何图案象征无限和神性。重复性质反映冥想和精神沉思。东海岸蜡染中常见的这些图案展示了马来-伊斯兰艺术融合。"""
        ...
        # Display the story with CLEAN TEXT (no markdown)
            ...
            if st.button("▶️ Generate & Play Audio", use_container_width=True):
                if st.session_state.current_story:
                    with st.spinner(f"Generating audio in {st.session_state.selected_language}..."):
                        try:
                            # Get language code
                            lang_code = language_codes_tts.get(st.session_state.selected_language, "en")
                            
                            # Get CLEAN story text (already clean without ** or *)
